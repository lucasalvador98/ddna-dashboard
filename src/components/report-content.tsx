...
const AXIS_COLORS: Record<string, { border: string; text: string; bg: string; bar: string }> = {
  pobreza:            { border: 'border-[#BF1363]', text: 'text-[#BF1363]', bg: 'bg-[#BF1363]/5',  bar: '#BF1363' },
  salud:              { border: 'border-[#10B981]', text: 'text-[#10B981]', bg: 'bg-[#10B981]/5',  bar: '#10B981' },
  educacion:          { border: 'border-[#F3A712]', text: 'text-[#F3A712]', bg: 'bg-[#F3A712]/5',  bar: '#F3A712' },
  inversion:          { border: 'border-[#3777FF]', text: 'text-[#3777FF]', bg: 'bg-[#3777FF]/5',  bar: '#3777FF' },
  seguridad_justicia: { border: 'border-[#7C3AED]', text: 'text-[#7C3AED]', bg: 'bg-[#7C3AED]/5',  bar: '#7C3AED' },
  seguridad:          { border: 'border-[#7C3AED]', text: 'text-[#7C3AED]', bg: 'bg-[#7C3AED]/5',  bar: '#7C3AED' },
  demografia:         { border: 'border-[#6B7280]', text: 'text-[#6B7280]', bg: 'bg-[#6B7280]/5',  bar: '#6B7280' },
};
...
function KpiCard({ kpi }: { kpi: ReportKPI }) {
  ...
  const barPct = isPercent
    ? Math.min(numericValue, 100)
    : Math.min((numericValue / 20) * 100, 100); // ‰ scale: 20‰ = 100%
  ...
  const trendColor =
    kpi.trend === 'up'   ? (kpi.alert ? 'text-red-600'   : 'text-green-600') :
    kpi.trend === 'down' ? (kpi.alert ? 'text-green-600' : 'text-red-600')   :
    'text-gray-500';
...
function HighlightBars({ highlights }: { highlights: Array<{ type: string; text: string }> }) {
  ...
  return (
    <div className="mt-4 space-y-1.5">
      <p className="font-accent text-xs uppercase tracking-wider text-gray-400 mb-2">Comparativa de hallazgos</p>
      {parsed.map((h, i) => {
        const pct = Math.round((h.numericVal! / max) * 100);
        const barColor =
          h.type === 'negative' ? '#EF4444' :
          h.type === 'positive' ? '#10B981' : '#9CA3AF';
        return (
          <div key={i} className="flex items-center gap-2 text-xs font-body">
            ...
            <span className="text-gray-600 truncate">{h.numericVal}{h.text.match(/(%|‰)/)?.[0] ?? ''}</span>
          </div>
        );
      })}
    </div>
          ...
          {alertKpis.length > 0 && (
            <div className="mb-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {alertKpis.map((kpi, i) => <KpiCard key={`alert-${i}`} kpi={kpi} />)}
              </div>
            </div>
          )}
          {normalKpis.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {normalKpis.map((kpi, i) => <KpiCard key={`kpi-${i}`} kpi={kpi} />)}
              </div>
            </div>
          )}
        </section>
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-3">
              <div>
                ...
                <h3 className="font-display text-lg text-[#00074E] mt-0.5">
                  {section.title}
                </h3>
              </div>
            </div>
            ...
            {report.dataQuality.map((dq, dqIdx) => {
              ...
              return (
                <div key={`dq-${dqIdx}`} className="flex items-start gap-3 text-sm">
                  <span className={clsx('inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-accent font-semibold flex-shrink-0', style.bg, style.text)}>
                    {style.label}
                  </span>
                  <div>
                    ...
                    {dq.issues.length > 0 && (
                      <ul className="mt-1 space-y-0.5">
                        {dq.issues.map((issue, iIdx) => (
                          <li key={`dq-issue-${iIdx}`} className="text-gray-600 list-disc list-inside">
                            {issue}
                          </li>
                        ))}
                      </ul>
        ...
        <section className="border border-gray-200 rounded-lg p-4">
          ...
          <div className="space-y-3">
            {report.discrepancies.map((d, dIdx) => (
              <div
                key={`disc-${dIdx}`}
                className={clsx('border-l-4 rounded-r-lg p-3 text-sm', SEVERITY_STYLES[d.severity] || SEVERITY_STYLES.baja)}
              >
        ...
        <section className="border border-gray-200 rounded-lg p-4">
          ...
          <div className="space-y-3">
            {report.crossReferences.map((cr, crIdx) => {
              ...
              return (
                <div key={`cr-${crIdx}`} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                  <span className={clsx('inline-flex items-center px-2 py-0.5 rounded text-xs font-accent font-semibold flex-shrink-0', badge.bg)}>
                    {badge.label}
                  </span>
        ...
        <section className="border border-gray-200 rounded-lg p-4">
          ...
          <ul className="space-y-2">
            {report.suggestedImprovements.map((imp, iIdx) => (
              <li key={`imp-${iIdx}`} className="flex items-start gap-3 font-body text-gray-700 text-sm">
